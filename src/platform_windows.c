...
static void read_port_name(HDEVINFO set, SP_DEVINFO_DATA *dev, char *buf, size_t buflen)
{
    if (buflen) buf[0] = '\0';
...
static void read_devprop(HDEVINFO set, SP_DEVINFO_DATA *dev, DWORD prop, char *buf, size_t buflen)
{
    if (buflen) buf[0] = '\0';
    SetupDiGetDeviceRegistryPropertyA(set, dev, prop, NULL, (PBYTE)buf, (DWORD)buflen, NULL);
}
...
static int parse_instance_id(const char *id, uint16_t *vid, uint16_t *pid, char *serial, size_t serlen)
{
    if (serlen) serial[0] = '\0';
    ...
    *vid = (uint16_t)strtol(v + 4, NULL, 16);
    if (p) *pid = (uint16_t)strtol(p + 4, NULL, 16);
    ...
    if (last && strchr(last + 1, '&') == NULL) to_cstr(last + 1, serial, serlen);
...
splist_status_t splist_backend_enumerate(splist_port_t **out_ports, size_t *out_count)
{
    ...
    *out_count = 0;
    ...
    if (set == INVALID_HANDLE_VALUE) return SPLIST_ERR_IO;
    ...
    for (DWORD i = 0; SetupDiEnumDeviceInfo(set, i, &dev); ++i) {
        ...
        if (port_name[0] == '\0') continue; /* not an addressable COM port */
        ...
        if (SetupDiGetDeviceInstanceIdA(set, &dev, instance_id, sizeof(instance_id), NULL)) {
            if (strncmp(instance_id, "USB", 3) == 0) {
                uint16_t vid = 0, pid = 0;
                if (parse_instance_id(instance_id, &vid, &pid, port.serial_number, sizeof(port.serial_number))) {
                    port.transport = SPLIST_TRANSPORT_USB;
                    port.vid = vid;
                    port.pid = pid;
                    port.has_usb_ids = 1;

                    read_devprop(set, &dev, SPDRP_MFG, port.manufacturer, sizeof(port.manufacturer));
                    read_devprop(set, &dev, SPDRP_FRIENDLYNAME, port.product, sizeof(port.product));
                }
